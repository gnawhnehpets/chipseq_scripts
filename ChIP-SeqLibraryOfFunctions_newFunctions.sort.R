...
# #                     print(paste0("x: ", x))
# #                     print(paste0("genelist.info.u: ", genelist.info.u[x,"Gene"]))
#                     
...
#                })
#                covchr <- cov.chr$gene.cov
#                genenames <- cov.chr$names
#                print(paste0("class.cov: ", class(cov.chr)))
#                return(covchr)
#           }
#      })
#      coverage <- do.call(cbind, newlist)
#      
#      print(paste0("class2: ", class(coverage)))
#      print(paste0("dim2: ", dim(coverage)))
# }
                    ...
                    if(which.rowsideann=="sort"){
                         ...
                         if(grepl("^C10D", i)){
                              if(grepl("^C10DH3K4", i)){
                                   ...
                                   save(C10DH3K4.order, file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_h3k4.order.Rdata"))     
                                   custom.order <- C10DH3K4.order
                              }
                              if(grepl("^C10DH3K27", i)){
                                   C10DH3K27.order <- order(rowSums(x.trnposed))
                                   save(C10DH3K27.order, file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_h3k27.order.Rdata"))     
                                   custom.order <- C10DH3K27.order
                              }
                              if(grepl("^C10DEZH2", i)){
                                   C10DEZH2.order <- order(rowSums(x.trnposed))
                                   save(C10DEZH2.order, file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_ezh2.order.Rdata"))     
                                   custom.order <- C10DEZH2.order
                              }
                              if(grepl("^C10DH3_", i)){
                                   C10DH3.order <- order(rowSums(x.trnposed))
                                   save(C10DH3.order, file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_h3.order.Rdata"))     
                                   custom.order <- C10DH3.order
                              }
                              if(grepl("^C10DDNMT1", i)){
                                   C10DDNMT1.order <- order(rowSums(x.trnposed))
                                   save(C10DDNMT1.order, file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_dnmt1.order.Rdata"))     
                                   custom.order <- C10DDNMT1.order
                              }
                              if(grepl("^C10DInput", i)){
                                   C10DInput.order <- order(rowSums(x.trnposed))
                                   save(C10DInput.order, file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_input.order.Rdata"))     
                                   custom.order <- C10DInput.order
                              }
                         }else{
                              if(grepl("H3K4", i)){
                                   load(file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_h3k4.order.Rdata"))     
                                   custom.order <- C10DH3K4.order
                              }
                              if(grepl("H3K27", i)){
                                   load(file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_h3k27.order.Rdata"))     
                                   custom.order <- C10DH3K27.order
                              }
                              if(grepl("EZH2", i)){
                                   load(file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_ezh2.order.Rdata"))     
                                   custom.order <- C10DEZH2.order
                              }
                              if(grepl("H3_", i)){
                                   load(file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_h3.order.Rdata"))     
                                   custom.order <- C10DH3.order
                              }
                              if(grepl("DNMT1", i)){
                                   load(file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_dnmt1.order.Rdata"))     
                                   custom.order <- C10DDNMT1.order
                              }
                              if(grepl("Input", i)){
                                   load(file=paste0("./",genelist.name,"/c10d_",genelist.name,"_",bin,"bp_input.order.Rdata"))     
                                   custom.order <- C10DInput.order
                              }
                         }

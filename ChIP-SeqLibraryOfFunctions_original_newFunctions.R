...
#   
  ...
  newlist <- lapply(chr, FUN=function(u){
    ...
    #Get coverage data only if there is a gene in a particular iteration of a chromosome. 'if' condition used here so that the script does not stop here  with an error if there is no gene in the query belonging to a particular chromosome.
    if(dim(genelist.info.u)[1] > 0){
      ...
      cov.chr <- sapply(1:nrow(genelist.info.u), FUN=function(x){
           ...
           if(genelist.info.u[x, "Strand"] == 1){
                ...
                #pick the bins up and downstream of the TSS window (set to the bin size, i.e. the bin contaiing the TSS coordinate is considered the central bin, and the coverage in the required number of bins up- and down-stream from the central bin will be extracted)
                
                gene.coverage <- coverage.data.u[(coverage.data.u[,"start"] >= up - bin) & (coverage.data.u[,"end"] < down + bin), "Seq_tags"]
                #The number of bins extracted for gene.coverage should be equal to num.bins. For eg. for 10000bp and 200 bp bin, it will be 51. If for some reason the number of coverage bins do not correspond to this number, remove them from the analysis (thus retain only genes having number of coverage bins= num.bins). Note that the coverage.data should have start-end columns of the form 0-200, 200-400, 400-600 (and not 0-199, 200-399, etc) for the >= and < operators to pick exactly the required number of bins.
                #if the TSS is at an end of a chromosome (or the data for that region is lacking), then report these as NA's so that the plotting functions later on work.
...
fun.average_heat.plots <- function(genelist.info, 
  ...
  if(whichgenelist=="stable.10M"){
       load(paste0("/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/Michelle/BED_files/Coverage_TSS_",bin,"bp_bin/normalizedBED_",bin,"bp_bin/outputdir/c10d_stable.10M_",bin,"bp_ann.bar.Rdata"))
       custom.annotation <- c10d.10M.ann.bar
  }
  if(whichgenelist=="intermediate.10M"){
       load(paste0("/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/Michelle/BED_files/Coverage_TSS_",bin,"bp_bin/normalizedBED_",bin,"bp_bin/outputdir/c10d_intermediate.10M_",bin,"bp_ann.bar.Rdata"))
       custom.annotation <- c10d.10M.ann.bar
  }
  if(whichgenelist=="stable.10M.new"){
       load("/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/Michelle/BED_files/Coverage_TSS_200bp_bin/normalizedBED_200bp_bin/outputdir/c10d_new_stable.10M_200bp_ann.bar.Rdata")
       custom.annotation <- c10d.10M.ann.bar
  }
  if(whichgenelist=="intermediate.10M.new"){
       load(paste0("/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/Michelle/BED_files/Coverage_TSS_",bin,"bp_bin/normalizedBED_",bin,"bp_bin/outputdir/c10d_new_intermediate.10M_",bin,"bp_ann.bar.Rdata"))
       custom.annotation <- c10d.10M.ann.bar
  }
...
head(x.trnposed)
        ...
        heatmap.3(x.trnposed, Rowv=T, Colv=F, scale="none", col=numberOfColors.x.trnposed, RowSideColors=custom.annotation, trace="none", dendrogram="row", cexRow=0.2, main="Heatmap of raw seq reads", na.rm=TRUE)
#         heatmap.3(x.trnposed, Rowv=T, Colv=F, scale="none", col=numberOfColors.x.trnposed, RowSideColors=row.annotation.color, trace="none", dendrogram="row", cexRow=0.2, main="Heatmap of raw seq reads", na.rm=TRUE)
#         heatmap.3(x.trnposed, Rowv=T, Colv=F, scale="none", col=numberOfColors.x.trnposed, RowSideColors=row.annotation.color, trace="none", dendrogram="row")
        dev.off()

        
        ## Ratio heatmap
        jpeg(heat_plot_RATIO, height = 500, width = 800, quality=100)
#         heatmap.2(ratioToInp.x.trnposed, Rowv=T, Colv=F, scale="none", col=numberOfColors.ratioToInp.x.trnposed, trace="none", dendrogram="row", cexRow=0.2, main="Heatmap of ratios\nof seq reads to average input")
        heatmap.3(ratioToInp.x.trnposed, Rowv=T, Colv=F, scale="none", col=numberOfColors.ratioToInp.x.trnposed, trace="none", RowSideColors=custom.annotation, dendrogram="row", cexRow=0.2, main="Heatmap of ratios\nof seq reads to average input", na.rm=TRUE)
        dev.off()
        #heatmap.2(x.trnposed, Rowv=F, Colv=F, scale="none", col=colorRampPalette(c("black", "white", "green"))(599), breaks=c(seq(-200, -120, length=200),seq(-120,120,length=200), seq(120, 200, length=200)), trace="none", dendrogram="none", symkey=T, symbreaks=T, cexCol=1.2) # From Hsing-Chen
      } else {
